...
export function orderMessage(order, items, feeCents, payLabel = "efectivo") {
  ...
  const when = order.scheduled_for
    ? `\n⏰ PROGRAMADO PARA LAS ${new Intl.DateTimeFormat("es-ES", { timeZone: "Europe/Madrid", hour: "2-digit", minute: "2-digit" }).format(new Date(order.scheduled_for))}\n`
    : "";
  return (
    `🛎️ NUEVO PEDIDO ${order.code}\n` + when + `\n` +
    lines.join("\n") +
    fee +
    `\n\nTOTAL: ${(order.total_cents / 100).toFixed(2)}€ (${payLabel})` +
