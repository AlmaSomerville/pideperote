...
function StripeRow({ r, reload }) {
  ...
  async function call(action) {
    ...
    try {
      ...
      if (d.url) {
        try { await navigator.clipboard.writeText(d.url); setMsg("Enlace de alta copiado — mándaselo por WhatsApp al bar."); }
        catch { prompt("Copia el enlace de alta:", d.url); }
      }
      if (action === "status")
        setMsg(d.charges_enabled ? "✅ Verificado: ya puede cobrar." : d.details_submitted ? "⏳ Datos enviados, Stripe verificando." : "⏳ Aún no ha completado el alta.");
      await reload();
    } catch (e) { setMsg("⚠️ " + e.message); }
    setBusy(false);
  }

  const estado = !r.stripe_account_id
    ? "Sin Stripe"
    : !r.stripe_charges_enabled
    ? "⏳ Alta pendiente"
    : r.online_payments
    ? "✅ Cobrando online"
    : "✅ Listo (el bar aún no lo activó)";

  return (
    <div className="commission-row">
      <span className="muted">Stripe:</span>
      <span className="tag">{estado}</span>
      {!r.stripe_account_id ? (
        <button className="btn small green" disabled={busy} onClick={() => call("connect")}>Conectar Stripe</button>
      ) : (
        <>
          <button className="btn small secondary" disabled={busy} onClick={() => call("link")}>Copiar enlace de alta</button>
          <button className="btn small secondary" disabled={busy} onClick={() => call("status")}>Comprobar estado</button>
        </>
      )}
