...
export async function createSchema() {
  ...
  // Comisión de PidePerote sobre pagos online: puntos básicos (250 = 2,50%) + céntimos fijos por pedido
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS commission_bps INT DEFAULT 0`;
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS commission_fixed_cents INT DEFAULT 0`;
  // Cuenta de Stripe Connect del restaurante (se rellena al conectarlo; vacío = sin pagos online)
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS stripe_account_id TEXT DEFAULT ''`;
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS stripe_charges_enabled BOOLEAN DEFAULT FALSE`;
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS stripe_onboard_token TEXT DEFAULT ''`;
  await sql`ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS online_payments BOOLEAN DEFAULT FALSE`;
  ...
  // Pagos online (Stripe): 'efectivo' u 'online'; los online no se muestran al bar hasta estar pagados
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS pay_method TEXT DEFAULT 'efectivo'`;
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS paid_online BOOLEAN DEFAULT FALSE`;
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS stripe_session_id TEXT DEFAULT ''`;
